...
```{bash, eval=FALSE}
snippy --outdir varcall --ref wildtype.fna -R1 mutant_R1.fastq -R2 mutant_R2.fastq --ram 2
```
...
First we need to move some files to your local computer. Move the file `snps.bam` from the snippy analysis, and the files `wildtype.fna` and `wildtype.gff` from the variant calling practical directory.
