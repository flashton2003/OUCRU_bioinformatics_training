...
Use the `cd` command to change directory.</div><br>
...
**Q1 What tests do your files pass? What do they fail? Think about what might cause them to fail. Which file has the best quality reads?**
...
**Q2: How many reads had adapters trimmed from the ends?**

**Q3: What command would you run to run fastqc on the trimmed output?**
<div class="toggle"><button>Hint</button>Take your fastqc command line above and change the read files to out_1.fastq and out_2.fastq. Remember you need to make the output directory before you run fastqc.</div><br>
...
<div class="toggle"><button>Hint</button>Use the `less` command to open a text file.</div><br>
...
**Q4: What are the numbers in these columns? Which GC windows are overrepresented (a score above 1)? Which are underrepresented (a score below 1)?**
...
**Q5: what is the -O option doing in the command line above?**
<div class="toggle"><button>Hint</button>Run `cutadapt --help` to see the help file.</div><br>

**Q6: Try running `cutadapt` with a different `-O` value. What happens if you vary the -O parameter? Why?**

**Q7: how would you modify the command to remove low-quality bases from the end of your reads?**
