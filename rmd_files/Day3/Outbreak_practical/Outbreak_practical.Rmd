...
* Navigate to `~/data/outbreak_practical/dataset1`. 

* Run `ls` in this directory. This directory contains the alignment of all the high quality variable sites found in the core genome for 19 *V. cholera* genomes from Haiti and 20 other *V. cholera* genomes from around the world by `snippy-core`.
...
Answer - 
Information on `disty` is [here](https://github.com/c2-d2/disty) and histogram.py is [here](https://github.com/bitly/data_hacks). `process_disty_output.py` is a script I wrote for data format conversion.
...
Answer - 1. See "BEST NUMBER OF THREADS: 1" in IQTREE log. Because I/O takes time, so it is not always the quickest to divide tasks between different processors.
...
Answer - example tree is [here](https://microreact.org/project/TuDufF5vs)
...
**Question 10:** Is there phylogeographical signal in the *V. cholerae* genomes from Haiti? You will have to zoom in on Haiti to see this. 
...
* Navigate to `~/data/outbreak_practical/dataset2`. 

* Run `ls` in this directory. This directory contains the alignment of all the high quality variable sites found in the core genome for 19 *V. cholerae* genomes from Haiti and 103 other *V. cholerae* genomes from around the world by `snippy-core`. In terms of the inclusion from other countries, we have focussed on isolates from the Indian sub-continent.
...
* Then we need to run a couple of commands to convert the tree to a good format to visualise (we will midpoint root the tree and order the nodes to be descending). For this, we will use a custom python script which uses the [ete3](http://etetoolkit.org/docs/latest/index.html) library and the very useful [Newick Utilities](http://cegg.unige.ch/newick_utils) package.

```{bash, eval=FALSE}
cat dataset2.aln.treefile | python ~/scripts/midpoint_root_tree.py | nw_order -c n - > dataset2.aln.pretty.tree
```

* Then download the `dataset2.aln.pretty.tree` onto your local computer and open it with `FigTree`. 

* In `FigTree`, go to `File -> Import annotations` in the menu bar, and navigate to the `outbreak_practical/dataset2/annotation_figtree.txt` file. Select this file and press open. Then in the main `FigTree` window, go to `Tip Lables`, and in the drop down `Display` menu, select `ANNOT`. You should now be able to see tip labels containing information about continent, country and year on the tree.
...
Answer - the inferred ancestor of all the *V. cholerae* in our tree (not the reference genome).
